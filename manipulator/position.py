...
class SubtractionApp(QWidget):
    ...
    def initUI(self):
        ...
        self.start_heating_button = QPushButton('Start Heating', self)
        self.start_heating_button.clicked.connect(self.start_heating)

        heating_layout.addWidget(self.start_heating_button)
    ...
    def check_pipe_for_updates(self):
        if self.pipe.poll():
            ...
            elif isinstance(data, tuple) and data[0] == "UPDATE_FAI_POSITION":
                updated_fai = data[1]
                self.fai_current_position.setText(str(updated_fai))
            elif isinstance(data, tuple) and data[0] == "UPDATE_VALUE1_POSITION":
                updated_value1 = data[1]
                self.current_position.setText(str(updated_value1))
    ...
    def start_movement(self):
        ...
        try:
            ...
            current_x = float(self.xaxis_current_position.text())
            current_y = float(self.yaxis_current_position.text())
            current_fai = float(self.fai_current_position.text())
            ...
            # 发送运动指令并附带所有参数
            self.pipe.send((abs_value, sign_value, value1, value2, self.x_target, self.y_target, self.fai_target ,current_x ,current_y, current_fai))
